...
if __name__ == '__main__':
    ...
    # if args.dataset in ['TruthfulQA', 'SQuAD1', 'NarrativeQA']:
    #     data = dataset_loader.load(args.dataset, detectLLM=args.detectLLM)
    # elif args.dataset == 'All':
    #     data = dataset_loader.load(args.dataset, filename1=args.Mixcase_filename, filename2=args.transfer_filename, MGT_only_GPT=args.MGT_only_GPT, test_only = args.test_only, train_threshold = args.train_threshold, mixcase_threshold = args.mixcase_threshold, no_auc=args.no_auc, train_with_mixcase = args.train_with_mixcase, seed=args.seed)
    ...
    #     outputs.append(run_sentinel(data, DEVICE=DEVICE))
    # outputs.append(run_supervised_experiment(data, model='roberta-base-openai-detector',
    #                cache_dir=cache_dir, batch_size=batch_size, DEVICE=DEVICE, test_only = args.test_only, no_auc=args.no_auc, ckpt_dir=args.ckpt_dir))
    # outputs.append(run_supervised_experiment(data, model='Hello-SimpleAI/chatgpt-detector-roberta',
    #                cache_dir=cache_dir, batch_size=batch_size, DEVICE=DEVICE, pos_bit=1, test_only = args.test_only, no_auc=args.no_auc, ckpt_dir=args.ckpt_dir))
    ...
    # outputs.append(run_radar(data, DEVICE=DEVICE))
    ...
    with open(os.path.join(SAVE_PATH, f"benchmark_results.pkl"), "wb") as f:
        pkl.dump(outputs, f)
    print(outputs)
    with open(f"logs/{args.log_name}_{args.Mixcase_filename}l", "w") as wf:
        for row in outputs:
            print(1)
            json.dump(row, wf)
            wf.write("\n")
